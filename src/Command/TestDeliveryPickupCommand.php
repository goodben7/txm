...
class TestDeliveryPickupCommand extends Command
{
    ...
    {
        $this->setName('txm:test-delivery-pickup')
            ->setDescription('Test delivery pickup notifications via event dispatcher');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $delivery->setDescription('Colis de test pour notification de ramassage');
        ...
        $delivery->setAdditionalInformation('Informations supplémentaires pour le test de ramassage');
        $delivery->setTrackingNumber('PA-' . date('dmy') . date('Hi') . 'PCK');
        $delivery->setStatus(Delivery::STATUS_PICKUPED); // Statut ramassé
        $delivery->setDeliveryPerson($deliveryPerson); // Assignation du livreur
        $delivery->setPickupedAt(new \DateTimeImmutable('now')); // Date de ramassage
        $delivery->setPickupedBy('SYSTEM'); // Ramassé par le système
        
        try {
            // Déclencher l'événement de ramassage de livraison
            $event = new ActivityEvent($delivery, Delivery::EVENT_DELIVERY_PICKUPED);
            $this->eventDispatcher->dispatch($event, ActivityEvent::getEventName(Delivery::class, Delivery::EVENT_DELIVERY_PICKUPED));
            
            $io->success('Événement de ramassage de livraison déclenché avec succès!');
