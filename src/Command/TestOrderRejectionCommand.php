...
class TestOrderRejectionCommand extends Command
{
    ...
    {
        $this->setName('txm:test-order-rejection')
            ->setDescription('Test order rejection notifications via event dispatcher');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $order = new Order();
        $order->setStatus(Order::STATUS_REJECTED); // Statut rejeté
        $order->setCreatedAt(new \DateTimeImmutable('-1 day')); // Créée hier
        $order->setCreatedBy('SYSTEM');
        $order->setRejectedAt(new \DateTimeImmutable('now')); // Rejetée maintenant
        $order->setRejectedBy('SYSTEM'); // Rejetée par le système
        ...
        $order->setDescription('Commande de test pour notification de rejet');
        ...
        try {
            // Déclencher l'événement de rejet de commande avec une raison de rejet
            $rejectionReason = 'Produits indisponibles en stock';
            $event = new ActivityEvent($order, Order::EVENT_ORDER_REJECTED, null, $rejectionReason);
            $this->eventDispatcher->dispatch($event, ActivityEvent::getEventName(Order::class, Order::EVENT_ORDER_REJECTED));
            
            $io->success('Événement de rejet de commande déclenché avec succès!');
            $io->note('Raison du rejet: ' . $rejectionReason);
