...
class TestDeliveryCancellationCommand extends Command
{
    ...
    {
        $this->setName('txm:test-delivery-cancellation')
            ->setDescription('Test delivery cancellation notifications via event dispatcher');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $delivery->setDescription('Colis de test pour notification d\'annulation');
        ...
        $delivery->setAdditionalInformation('Informations supplémentaires pour le test d\'annulation');
        $delivery->setTrackingNumber('PA-' . date('dmy') . date('Hi') . 'CAN');
        $delivery->setStatus(Delivery::STATUS_CANCELED); // Statut annulé
        $delivery->setDeliveryPerson($deliveryPerson); // Assignation du livreur
        
        // Définir un message explicatif de l'annulation
        $delivery->setMessage('Annulation due à une indisponibilité du client');
        
        // Définir la date d'annulation
        $delivery->setCanceledAt(new \DateTimeImmutable('now'));
        $delivery->setCanceledBy('SYSTEM');
        
        try {
            // Déclencher l'événement d'annulation de livraison
            $event = new ActivityEvent($delivery, Delivery::EVENT_DELIVERY_CANCELED);
            $this->eventDispatcher->dispatch($event, ActivityEvent::getEventName(Delivery::class, Delivery::EVENT_DELIVERY_CANCELED));
            
            $io->success('Événement d\'annulation de livraison déclenché avec succès!');
            ...
            $io->table(
                ...
                [
                    ...
                    ['Raison de l\'annulation', $delivery->getMessage()],
                    ['Livreur assigné', $deliveryPerson->getFullname()],
                    ['Contact du livreur', $deliveryPerson->getPhone()],
                    ['Date d\'annulation', $delivery->getCanceledAt()->format('d/m/Y H:i')]
                ]
            );
