...
class TestOrderCreationCommand extends Command
{
    ...
    {
        $this->setName('txm:test-order-creation')
            ->setDescription('Test order creation notifications via event dispatcher');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $order->setDescription('Commande de test pour notification');
        ...
        try {
            // Déclencher l'événement de création de commande
            $event = new ActivityEvent($order, Order::EVENT_ORDER_CREATED);
            $this->eventDispatcher->dispatch($event, ActivityEvent::getEventName(Order::class, Order::EVENT_ORDER_CREATED));
            
            $io->success('Événement de création de commande déclenché avec succès!');
