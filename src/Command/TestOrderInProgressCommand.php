...
class TestOrderInProgressCommand extends Command
{
    ...
    {
        $this->setName('txm:test-order-inprogress')
            ->setDescription('Test order in-progress notifications via event dispatcher');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $order = new Order();
        $order->setStatus(Order::STATUS_IN_PROGRESS); // Statut en cours de traitement
        $order->setCreatedAt(new \DateTimeImmutable('-1 day')); // Créée hier
        $order->setCreatedBy('SYSTEM');
        $order->setValidatedAt(new \DateTimeImmutable('-2 hours')); // Validée il y a 2 heures
        $order->setValidatedBy('SYSTEM'); // Validée par le système
        $order->setInprogressAt(new \DateTimeImmutable('now')); // Mise en cours maintenant
        ...
        $order->setDescription('Commande de test pour notification de mise en cours');
        ...
        try {
            // Déclencher l'événement de mise en cours de commande
            $event = new ActivityEvent($order, Order::EVENT_ORDER_INPROGRESS);
            $this->eventDispatcher->dispatch($event, ActivityEvent::getEventName(Order::class, Order::EVENT_ORDER_INPROGRESS));
            
            $io->success('Événement de mise en cours de commande déclenché avec succès!');
            ...
            $io->table(
                ...
                [
                    ...
                    ['Date de mise en cours', $order->getInprogressAt()->format('d/m/Y H:i')]
                ]
            );
