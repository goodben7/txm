...
class TestOrderTerminationCommand extends Command
{
    ...
    {
        $this->setName('txm:test-order-termination')
            ->setDescription('Test order termination notifications via event dispatcher');
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $order = new Order();
        $order->setStatus(Order::STATUS_COMPLETED); // Statut terminé
        $order->setCreatedAt(new \DateTimeImmutable('-2 days')); // Créée il y a 2 jours
        $order->setCreatedBy('SYSTEM');
        $order->setValidatedAt(new \DateTimeImmutable('-1 day')); // Validée hier
        $order->setValidatedBy('SYSTEM'); // Validée par le système
        $order->setInprogressAt(new \DateTimeImmutable('-12 hours')); // Mise en cours il y a 12 heures
        $order->setInprogressBy('SYSTEM'); // Mise en cours par le système
        $order->setTerminedAt(new \DateTimeImmutable('now')); // Terminée maintenant
        $order->setTerminedBy('SYSTEM'); // Terminée par le système
        ...
        $order->setDescription('Commande de test pour notification de terminaison');
        ...
        try {
            // Déclencher l'événement de terminaison de commande
            $event = new ActivityEvent($order, Order::EVENT_ORDER_TERMINATED);
            $this->eventDispatcher->dispatch($event, ActivityEvent::getEventName(Order::class, Order::EVENT_ORDER_TERMINATED));
            
            $io->success('Événement de terminaison de commande déclenché avec succès!');
            ...
            $io->table(
                ...
                [
                    ...
                    ['Statut', 'Terminée'],
                    ['Magasin', $store->getLabel()],
                    ['Date de terminaison', $order->getTerminedAt()->format('d/m/Y H:i')]
                ]
            );
