...
class OrderTerminationNotifier implements EventSubscriberInterface {
    ...
    {
        return [
            ActivityEvent::getEventName(Order::class, Order::EVENT_ORDER_TERMINATED) => 'onOrderTermination',
        ];
    }
    ...
    public function onOrderTermination(ActivityEvent $event)
    {
        /** @var Order */
        ...
        try {
            ...
            if ($order->getCustomer()?->getEmail()) {
                ...
                $customerEmailNotification->setSubject('Commande terminée');
                $customerEmailNotification->setTitle('Commande terminée');
                $customerEmailNotification->setBody('La commande numéro : ' . ($order->getSerialNumber() ?? $order->getId()) . ' est maintenant terminée. L\'équipe Tinda a pris le relais sur la commande pour la récupérer et la livrer.');
                ...
                $customerEmailNotification->setData([
                    ...
                    'Date de terminaison' => $order->getTerminedAt() ? $order->getTerminedAt()->format('d/m/Y H:i') : 'Non disponible'
                ]);
            ...
            if ($order->getCustomer()?->getPhone()) {
                ...
                $customerWhatsappNotification->setSubject('Commande terminée');
                $customerWhatsappNotification->setTitle('Commande terminée');
                $customerWhatsappNotification->setBody('La commande numéro : ' . ($order->getSerialNumber() ?? $order->getId()) . ' est maintenant terminée. L\'équipe Tinda a pris le relais sur la commande pour la récupérer et la livrer.');
            ...
            if ($order->getUserId()) {
                ...
                if ($user?->getEmail()) {
                    ...
                    $userEmailNotification->setSubject('Votre commande est terminée');
                    $userEmailNotification->setTitle('Votre commande est terminée');
                    $userEmailNotification->setBody('Votre commande numéro : ' . ($order->getSerialNumber() ?? $order->getId()) . ' est maintenant terminée. L\'équipe Tinda a pris le relais sur votre commande pour la récupérer et la livrer.');
                ...
                if ($user?->getPhone()) {
                    ...
                    $userWhatsappNotification->setSubject('Votre commande est terminée');
                    $userWhatsappNotification->setTitle('Votre commande est terminée');
                    $userWhatsappNotification->setBody('Votre commande numéro : ' . ($order->getSerialNumber() ?? $order->getId()) . ' est maintenant terminée. L\'équipe Tinda a pris le relais sur votre commande pour la récupérer et la livrer.');
