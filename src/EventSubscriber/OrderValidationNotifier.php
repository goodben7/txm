...
class OrderValidationNotifier implements EventSubscriberInterface {
    ...
    {
        return [
            ActivityEvent::getEventName(Order::class, Order::EVENT_ORDER_VALIDATED) => 'onOrderValidation',
        ];
    }
    ...
    public function onOrderValidation(ActivityEvent $event)
    {
        /** @var Order */
        ...
        try {
            ...
            if ($order->getCustomer()?->getEmail()) {
                ...
                $customerEmailNotification->setSubject('Commande validée');
                $customerEmailNotification->setTitle('Commande validée');
                $customerEmailNotification->setBody("La commande numéro : " . ($order->getSerialNumber() ?? $order->getId()) . " a été validée.");
                ...
                $customerEmailNotification->setData([
                    ...
                    'Date de validation' => $order->getValidatedAt() ? $order->getValidatedAt()->format('d/m/Y H:i') : 'Non disponible'
                ]);
            ...
            if ($order->getCustomer()?->getPhone()) {
                ...
                $customerWhatsappNotification->setSubject('Commande validée');
                $customerWhatsappNotification->setTitle('Commande validée');
                $customerWhatsappNotification->setBody('La commande numéro : ' . ($order->getSerialNumber() ?? $order->getId()) . ' a été validée.');
            ...
            if ($order->getUserId()) {
                ...
                if ($user?->getEmail()) {
                    ...
                    $userEmailNotification->setSubject('Votre commande a été validée');
                    $userEmailNotification->setTitle('Votre commande a été validée');
                    $userEmailNotification->setBody("Votre commande numéro : " . ($order->getSerialNumber() ?? $order->getId()) . " a été validée.");
                ...
                if ($user?->getPhone()) {
                    ...
                    $userWhatsappNotification->setSubject('Votre commande a été validée');
                    $userWhatsappNotification->setTitle('Votre commande a été validée');
                    $userWhatsappNotification->setBody('Votre commande numéro : ' . ($order->getSerialNumber() ?? $order->getId()) . ' a été validée.');
