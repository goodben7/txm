...
class OrderCreationNotifier implements EventSubscriberInterface {
    ...
    {
        return [
            ActivityEvent::getEventName(Order::class, Order::EVENT_ORDER_CREATED) => 'onOrderCreation',
        ];
    }
    ...
    public function onOrderCreation(ActivityEvent $event)
    {
        /** @var Order */
        ...
        try {
            ...
            if ($order->getCustomer()?->getEmail()) {
                ...
                $customerEmailNotification->setSubject('Nouvelle commande reçue');
                $customerEmailNotification->setTitle('Nouvelle commande reçue');
                $customerEmailNotification->setBody("Vous avez reçu une nouvelle commande d'un client.");
                ...
                $customerEmailNotification->setData([
                    ...
                    'Date de création' => $order->getCreatedAt() ? $order->getCreatedAt()->format('d/m/Y H:i') : 'Non disponible'
                ]);
            ...
            if ($order->getCustomer()?->getPhone()) {
                ...
                $customerWhatsappNotification->setSubject('Nouvelle commande reçue');
                $customerWhatsappNotification->setTitle('Nouvelle commande reçue');
                $customerWhatsappNotification->setBody('Vous avez reçu une nouvelle commande d\'un client.');
            ...
            if ($order->getUserId()) {
                ...
                if ($user?->getEmail()) {
                    ...
                    $userEmailNotification->setSubject('Votre commande a été créée');
                    $userEmailNotification->setTitle('Votre commande a été créée');
                    $userEmailNotification->setBody("Votre commande a été créée avec succès.");
                ...
                if ($user?->getPhone()) {
                    ...
                    $userWhatsappNotification->setSubject('Votre commande a été créée');
                    $userWhatsappNotification->setTitle('Votre commande a été créée');
                    $userWhatsappNotification->setBody('Votre commande a été créée avec succès.');
